...
ExtrusionSimulationWidget::ExtrusionSimulationWidget(QWidget *parent) : BaseParamWidget(parent) {
    setupUi();
    applyCommonStyles();
}

void ExtrusionSimulationWidget::setupUi() {
    QVBoxLayout *mainLayout = createMainLayout(this);
    setupHeader("仿真控制参数 (挤压工艺)");
...
void ExtrusionSimulationWidget::setData(const ExtrusionSimulationData &d) {
...
void ExtrusionSimulationWidget::getData(ExtrusionSimulationData &d) const {
...
void ExtrusionSimulationWidget::setSaveCallback(std::function<void(const ExtrusionSimulationData&)> cb) {
    m_saveCallback = cb;
}
